/* 	Nombre     : 	client_udp_broadcast2.c
	Autor      : 	Luis A. Rivera
	Descripción: 	Cliente simple (broadcast), con bind().
					IE3048						*/
...
int main(int argc, char *argv[])
{
	...
	do
	{
		...
		if(buffer[0] != '!')
		{
			...
			// Como se hizo el bind, este programa recibirá los mensajes propios.
			// Por lo tanto, recibimos otra vez.
            memset(buffer, 0, MSG_SIZE);
			// receive message
			n = recvfrom(sockfd, buffer, MSG_SIZE, 0, (struct sockaddr *)&from, &length);
			if(n < 0)
